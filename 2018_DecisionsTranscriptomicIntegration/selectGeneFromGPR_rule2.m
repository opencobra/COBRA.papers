function [expressionCol gene_used] = selectGeneFromGPR_rule2(model, gene_names, gene_exp, parsedGPR)
% Map gene expression to reaction expression using the GPR rules. An AND
% will be replaced by MIN and an OR will be replaced by SUM.
%
% USAGE:
...
% AUTHOR: Anne Richelle, May 2017
    ...
    for i = 1:length(model.rxns)
        ...
        if ~isempty(curExpr)
            expressionCol(i)=sum(curExpr);%If there is data for any gene in the 'OR' rule, take sum the minimum values
            gene_used{i}=gene_potential;
        end
    end
gene_used=gene_used';
end