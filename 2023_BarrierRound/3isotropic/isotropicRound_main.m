...
for c = numPoly
    ...
    tic;
    P = isotropicRound(P); % resulting in a constraint-based rounded polytope
    roundTime = toc;
    fprintf("Loading complete (%d/%d): %s\n", c, length(numPoly), matfiles(c).name)
    
    % Find a full-dimensional description of this degenerate subspace
    P.roundTime = roundTime;
    
    if size(P.A, 2) < 6000
        save(fullfile(fileparts(curFolder), '/Instances/3testbed-isotropic/', matfiles(c).name), 'P')
    else
        save(fullfile(fileparts(curFolder), '/Instances/3testbed-isotropic/', matfiles(c).name), 'P', '-v7.3')
    end
end